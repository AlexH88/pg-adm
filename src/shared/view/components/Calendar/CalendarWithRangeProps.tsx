...
class Calendar extends React.PureComponent<IProps, IState> {
  ...
  public render() {
    const { label, onRemove, range} = this.props;
    ...
    return (
      <div className={b()}>
        <TextInput
          ...
          value={`${new Date(range.from*1000).getDate()}/${new Date(range.from*1000).getMonth()+1} - ${new Date(range.to*1000).getDate()}/${new Date(range.to*1000).getMonth()+1}`}
        />
