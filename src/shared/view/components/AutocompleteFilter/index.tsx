...
class AutocompleteFilter extends React.PureComponent<IOwnProps, IFilterState> {
...
/*
    if (source && source.length > 0) {
        source.forEach((s: any) => {
          if(s.name.toLowerCase().indexOf(this.state.currentQuery.toLowerCase()) > -1){
            processedSource.push(s[newFilterName]);
          }
        });
    }*/
  ...
  private handleKeyDown(e) {
    if (e.key === 'Enter') {
      let onQueryChange = this.getOptions;
      let onChange = this.onChange;
      let value = this.state.currentQuery;
  ...
  private onChange(value: any) {
    ...
    if (onChange) {
//      onChange(this.state.currentQuery);
      onChange(value);
    };
