...
import {IReduxState, ShowResourceActions} from '../../namespace';
import {initialResource, initialState} from '../data/initial';

function mainReducer(state: IReduxState = initialState, action: ShowResourceActions | any): /*IReduxState*/any {
  ...
  switch (action.type) {
    ...
    case 'SHOW_RESOURCE:LOAD_RESOURCE_SUCCESS': {
    ...
    case 'SHOW_RESOURCE:CHANGE_LOADER': {
      return imState
    ...
    // case 'SHOW_RESOURCE:CLEAR_TIMER_ID': {
      // debugger;
      // for(let i = 0; i < 1000000; i++) clearTimeout(i); // TODO Переделать срочно!
      // const timerId = imState.getIn([resource, 'timerId']);
    // }
    ...
    case 'SHOW_RESOURCE:SET_GALLERY': {
      const { payload : { jobId, processed } } = action;
      return imState.setIn(['jobs', 'gallery', jobId], processed).toJS();
    }

    case 'SHOW_RESOURCE:LOAD_BASE_TEMPLATE_SUCCESS': {
      const { payload } = action;
      return imState
        .setIn([resource, 'baseTemplate'], payload)
        .toJS();
    }
