...
function mapStateToProps(state: IReduxState): IStateProps {
  ...
  return {
    ...
    headers: ShowResourceSelectors.getHeaders(state, 'printer_api_events'),
    configHeaders: getHeadersConfig('printer_api_events', state),
    cofigAggregates: getAggregates('printer_api_events', state),
  };
}
...
class Errors extends React.PureComponent<IDispatchProps&IStateProps, {}> {

  private b = block('errors');
  ...
  public render() {
    ...
    return (
        <div className={this.b()}>
          <FilterResource configs={configHeaders} resource="printer_api_events" label="События > Ошибки"/>
          <ShowResource
              resource="printer_api_events"
              headers={headers}
              configs={{}}
...
export { Errors };
export default connect<IStateProps, IDispatchProps, {} >(mapStateToProps, mapDispatchToProps)(Errors);