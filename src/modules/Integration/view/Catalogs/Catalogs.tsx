...
function mapStateToProps(state: IReduxState): IStateProps {
  ...
  return {
    ...
    modalMode: state.integration.modalMode,
    showRemoveModal: state.integration.showRemoveModal,
    ...
    if (field === 'enabled') {
      return (
        <div style={{ display: 'flex', alignItems: 'center' }}>
          {row['connection']
            ? <span className={b('circle', { green: true })} id="catalog-green"/>
            : <span className={b('circle', { red: true })} id="catalog-red"/>
          }
          <span id="catalog-switch">
            <Switch
              theme={theme}
              checked={row[field]}
              onChange={() => this.handleToggleCatalog(row.id, row[field])}
              className="catalog_catalog-switch"
            />
            {/*
            <ProgressBar
              theme={theme}
        ...
        <div>
          {
            currentOperator['catalogs.put']
              ? <ActionFormatter icon="pen" onClick={editCatalog} id={row.id} mode="edit" title="Редактировать" idHash={`integration-catalog-pen-${generateIdElement()}`}/>
              : null
          }
          {
            currentOperator['catalogs.delete']
              ? <ActionFormatter icon="trash" onClick={deleteCatalog} id={row.id} title="Удалить" idHash={`integration-catalog-trash-${generateIdElement()}`}/>
              : null
          }
        </div>
      );
