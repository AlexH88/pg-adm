...
const resource = 'report_by_group_printers';
...
function mapStateToProps(state: IReduxState): IStateProps {
  ...
  if(state.showResource.hasOwnProperty('report_by_group_printers')){
    if(state.showResource.report_by_group_printers.hasOwnProperty('data')){
      if(state.showResource.report_by_group_printers.data.hasOwnProperty('0')){
        if(state.showResource.report_by_group_printers.data[0].length > 0){
          isEmptyReport = true
        }
      }
    }
...
function mapDispatchToProps(dispatch: any): IDispatchProps {
  return bindActionCreators({
    switchModalStatus: injectResource('report_by_group_printers', actions.switchModalStatus),
    downloadReport: injectResource('report_by_group_printers', actions.downloadReport),
    clearSettings: injectResource('reports', actions.clearSettings),
  }, dispatch);
}

class ReportByGroupPrinters extends React.PureComponent<IStateProps & IDispatchProps, {}> {
    ...
    return (
      <div className={b()}>
        <FilterResource
          ...
          resource={resource}
          label='Отчет по группам принтеров'
...
export { ReportByGroupPrinters };
export default connect<IStateProps, IDispatchProps, {} >(mapStateToProps, mapDispatchToProps)(ReportByGroupPrinters);
