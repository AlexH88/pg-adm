...
const resource = 'report_by_printers';
...
function mapStateToProps(state: IReduxState): IStateProps {
  ...
  if(state.showResource.hasOwnProperty('report_by_printers')){
    if(state.showResource.report_by_printers.hasOwnProperty('data')){
      if(state.showResource.report_by_printers.data.hasOwnProperty('0')){
        if(state.showResource.report_by_printers.data[0].length > 0){
          isEmptyReport = true
        }
      }
    }
...
function mapDispatchToProps(dispatch: any): IDispatchProps {
  return bindActionCreators({
    switchModalStatus: injectResource('report_by_printers', actions.switchModalStatus),
    downloadReport: injectResource('report_by_printers', actions.downloadReport),
    clearSettings: injectResource('reports', actions.clearSettings),
  }, dispatch);
}

class ReportByPrinters extends React.PureComponent<IStateProps & IDispatchProps, {}> {
...
export { ReportByPrinters };
export default connect<IStateProps, IDispatchProps, {} >(mapStateToProps, mapDispatchToProps)(ReportByPrinters);
