...
function mapStateToProps(state: IReduxState): IStateProps {
  ...
  if(state.showResource.hasOwnProperty('reports')){
    if(state.showResource.reports.hasOwnProperty('data')){
      if(state.showResource.reports.data.hasOwnProperty('0')){
        if(state.showResource.reports.data[0].length > 0){
          isEmptyReport = true
        }
      }
    }
...
function mapDispatchToProps(dispatch: any): IDispatchProps {
  return bindActionCreators({
    ...
    downloadReport: injectResource('reports', actions.downloadReport),
    clearSettings: injectResource('reports', actions.clearSettings),
  }, dispatch);
}

class Report extends React.PureComponent<IStateProps & IDispatchProps, {}> {
...
export { Report };
export default connect<IStateProps, IDispatchProps, {} >(mapStateToProps, mapDispatchToProps)(Report);
